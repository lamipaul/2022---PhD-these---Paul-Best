...
\subsection{Spectral features (humpback whale calls)}\label{chap:HB_annot}
\textit{This work was conducted on the Carimam dataset (see section~\ref{chap:data_Toulon}).}\\
To isolate vocalisations from the rest of the data (void, reef activity, boats, self noise of the sound card), spectral features of recording chunks (few seconds long) were extracted. They consist in temporally downsampled spectrograms with sorted frequency bins to reflect their energy distribution.

Downsampling is done to keep as few features as possible (to avoid the curse of dimensionality during clustering). Moreover, to make abstraction of the temporal information (whether a signal is at the beginning of a chunk or at its end), we sort each frequency bin in descending order. This also allows, in a second step, to select specific columns: the first denoting the highest energy in the chunk for each frequency bin, and the last their lowest. Looking for short events for instance, we might want to select the first and second columns. Chunks with a large gap between the two should thus contain a short but strong acoustic event. Like so, it could be differentiated from chunks with stationary strong energy and chunks with a low overall energy, and this for specific frequency bins. For simplicity, this set of columns to be kept will be referred to as `quantiles'.
...
\begin{figure}
    ...
    \caption{Interface for browsing clusters. The left panel displays clustered \ac{UMAP} projections of audio chunks spectral features, with red dots signaling points that have been clicked on. The right panel displays the spectrogram of the last selected audio as well as its meta data.}

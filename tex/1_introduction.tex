...
\begin{itemize}
    ...
    \item Chapter \ref{chap:conservation} first illustrates the application of a \acs{CNN} model in a real-time alert system for collision risk mitigation. Then, again using a \acs{CNN} based detection system, a long term survey of Mediterranean sperm whales is analysed, revealing presence patterns in relationship with ferries traffic.
    
    \item Chapter \ref{chap:com} first characterises the long term evolution of the Mediterranean fin whale song (obviously based on \acs{CNN} detections) and puts it in relation to other populations worldwide. Finally, as a last contribution to this thesis, orca call sequences are studied (at a larger scale than ever before), advancing questions on their potential structure.
\end{itemize}
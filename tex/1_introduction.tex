...
\subsubsection{Research problematic}

Given the context described above, this thesis oriented its work around the following problematic: ``how to best train deep learning models to solve cetacean vocalisation detection and classification tasks''.


\subsubsection{Structure of the manuscript}
...
Following the \ac{SOTA} and material description, each chapter of this manuscript puts forward several contributions to the field of \ac{PAM} using \acp{ANN}:

\begin{itemize}
    \item Chapter \ref{chap:annotation} proposes several reusable interfaces making annotating easier and quicker (they allowed to gather 5 databases, each of thousand(s) samples). One of these makes use of an auto-encoder which appears to be a relevant feature extractor for orca call type clustering.
    
    \item Chapter \ref{chap:training} presents a light weight but effective \acs{CNN} architecture to be embedded in low power \acsp{MCU}. Also, it gives insights on of \acs{PCEN} behaviour, and reports on the resolution of 6 different tasks using deep learning (5 of detection and 1 of classification). Finally, it proposes a new performance metric for detection systems.
    
    \item Chapter \ref{chap:conservation} illustrates the application of a \acs{CNN} model in a real-time alert system for collision risk mitigation. Then, again using a \acs{CNN} based detection system, a long term survey of Mediterranean sperm whales is analysed, revealing presence patterns in relationship with ferries traffic.
    
    \item Chapter \ref{chap:com} characterises the long term evolution of the Mediterranean fin whale song (obviously based on \acs{CNN} detections) and puts it in relation to other populations worldwide. Finally, as a last contribution to this thesis, orca call sequences are studied (at a larger scale than ever before), advancing questions on their potential structure.
\end{itemize}
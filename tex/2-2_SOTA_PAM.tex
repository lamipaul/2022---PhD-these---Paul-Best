...
Let us start with an acoustic recording. It is described digitally by a sequence of samples \(\mathbf{x}=\{\mathbf{x}_i\}_{1..n}\) that denotes the evolution of pressure through time. The number of samples recorded per second is given by the sampling frequency, noted $f_s$.


\subsubsection{Fourier}

The Fourier transform is a major tool in signal processing. It allows to describe any signal as a sum of sinuses, each characterised by an amplitude and a phase. This representation is called the spectrum. Given our acoustic signal \(\mathbf{x}\), the \ac{DFT} will yield a spectrum \(\mathbf{X}_f\) that gives complex numbers as a function of frequencies. These complex numbers describe each frequency component of the signal, with the amplitude as the modulus and phase as the angle. The behavior of the \ac{DFT} \(\mathcal{F}\) of a signal of size $N$ is described by Eq.~\ref{eq:fourier}.

\begin{equation}\label{eq:fourier}
     \mathbf{X}_f = \mathcal{F}(\mathbf{x}) = \sum_{n = 0}^{N} x[n] e^{-i\frac{2\pi }{N} fn}.
\end{equation}
...
\begin{equation}
     \mathbf{S}_{f,t} = \lvert \sum_{n = 0}^{NFFT} \mathbf{x}[t \times hop + n] e^{-i\frac{2\pi }{NFFT} fn} \rvert ^2. 
...
Several parameters are to be set prior to the \ac{STFT} computation, that define the sliding window's behavior: the window size $NFFT$ and the hop size \(hop\). Along with the sampling frequency \(f_s\), these will define the range and resolution of our resulting spectrogram :
...
\begin{itemize}
    ...
    \item The window size will define the number of frequency bins of the spectrogram: \(\Delta f =\frac{f_s}{NFFT}\). \\ A bigger window will thus yield a more detailed representation of the signal, but also might blur short transitory events (the yielded spectrum is an average of the frequency contents in the window). Also note that a bigger \(NFFT\) implies more computation per \ac{DFT}.
    ...
    \item Zero padding allows to increase the sampling rate of the \ac{DFT} while preserving short events in the window from being blurred. It works by adding zeros at the border of the \ac{DFT} window.
...
Sometimes, both temporal and frequency high resolutions are needed, and a satisfying window size does not exist. This motivates the use of other bases than sinuses for spectral transforms such as wavelets (Gaussian enveloped sinuses for instance). 
...
\begin{figure}
    ...
    \caption{Example of Mel filterbank (set of triangular filters). Each color represents a filter, which will ponderate the input spectrum to yield a Mel frequency bin.}
    \label{fig:mel}
\end{figure}
...
\begin{equation}\label{eq:filter}
    \mathbf{M}_{f,t} = (1-s)\mathbf{M}_{f,t-1} + s\mathbf{S}_{f,t},
\end{equation}

\begin{equation}\label{eq:pcen}
    \mathbf{PCEN}_{f,t} = \left(\frac{\mathbf{S}_{f,t}}{(\epsilon+\mathbf{M}_{f,t})^\alpha} + \delta \right)^r - \delta^r.
\end{equation}
